...
// Vertex Shader
//--------------------------------------------------------------------------------------
VS_OUTPUT VS( float4 Pos : POSITION, float4 nor : NORMAL, float2 myTex : TEXCOORD )
{
    VS_OUTPUT output;
...
float4 PS( VS_OUTPUT input ) : SV_Target
{
	...
	ComputeSpotLight(Mater, SpLight[0], input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);

	ambient += A;
	diffuse += D;
	spec += S;

	ComputeSpotLight(Mater, SpLight[1], input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);

	ambient += A;
	diffuse += D;
	spec += S;

	ComputeSpotLight(Mater, SpLight[2], input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);

	ambient += A;
	diffuse += D;
	spec += S;

	ComputeSpotLight(Mater, SpLight[3], input.PosMS, normalize(input.Norm), msDirToEye, A, D, S);
	...
	if (_fogRange != 0)
	{
		...
		//float FogStart = _fogStart;	// anything closer than this value will have no fog
		//float FogRange = _fogRange;	// fog contribution increases linearly until full fog at dist = FogStart + Range
		////float4 FogColor = float4(.2,.2,.2, 1); // grey
		//float4 FogColor = _fogColor; // MidnightBlue

		// Compute fog density as function of distance to eye
		float distToEye = length(msEyePos - input.PosMS);
		float FogPercent = saturate((distToEye - _fogStart) / _fogRange);  // See https://msdn.microsoft.com/en-us/library/windows/desktop/bb509645(v=vs.85).aspx

		// blend litColor with fog level
		// C_dst = litColor
		// C_src = FogColor
		// C = C_dst * (1 - FogPercent) + C_src * FogPercent
		litColor = litColor * (1 - FogPercent) + _fogColor * FogPercent;
	}
